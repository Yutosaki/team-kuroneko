...
この「常に全件走査してしまう」という課題を解決するため、本章では上記の条件を評価し、最も効率的な検索手順を自動的に決定する@<b>{プランナ（Planner）}を導入します。

具体的には、以下の順序で実装を進めます。

 1. @<b>{実行計画とプランナの導入}: 検索手順を決定する「プランナ」と、実際のデータ操作を行う「クエリエグゼキュータ」の役割を明確に分離します。
 2. @<b>{カラムへのインデックス定義とカタログ保存}: @<code>{CREATE TABLE} でインデックスを指定できるようにし、テーブル定義（メタデータ）として保存・管理します。
 3. @<b>{インデックスの構築と同期処理}: 第5章で作ったB-Treeを利用し、データの追加・更新・削除に合わせてインデックスを自動で同期する仕組みを作ります。
 4. @<b>{プランナによる実行方法（経路）の選択}: SQLの解析結果（Statement）とテーブル定義（Schema）から@<b>{実行計画（Execution Plan）}を生成し、全件走査（SeqScan）とインデックス検索（IndexScan）のどちらが最適かを判断するロジックを実装します。

このステップを踏むことで、ユーザーが内部のアルゴリズムを意識しなくても、データベース側が自動でB-Treeを活用して高速な検索を行ってくれるようになります。まずは、全体の処理の流れをどのように分離するのかを見ていきましょう。
...
それは、現在の実装では、@<b>{抽出したレコードをすべてメモリ上の} @<code>{List<Row>} @<b>{に一気に保持してから、絞り込みや結合を行っている}という点です。データ量が増加すると、このリストがメモリを過大に消費し、いずれメモリ枯渇（Out Of Memory）を引き起こしてデータベースがクラッシュしてしまいます。
