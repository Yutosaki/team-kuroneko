...
第1章のロードマップでは、第6章を「テーブル定義とSQLの実行」を導入する章と位置付けました。本章では、SQLによって目的を記述し、データベース側がその内容を読み取って処理する入口を作ります。SQLを利用した経験があれば読み進められるように、言語処理の用語はTokenizer、Parser、ASTの順に、実際の変換過程と対応させながら説明します。
