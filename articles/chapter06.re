...
一方、第5章までのデータベースには、利用者から見た二つの課題が残っています。一つ目は、@<code>{id}と@<code>{name}という決められたカラムを前提としていることです。別のカラムやデータ型を扱うには、保存形式とプログラム自体を変更しなければなりません。

二つ目は、データを操作するために独自コマンドの書き方と実行手順を知る必要があることです。利用者が「どのデータが欲しいか」だけでなく、検索メソッドの呼び出し方まで意識する構造では、用途が増えるたびに操作方法も増えてしまいます。

第1章のロードマップでは、第6章を「テーブル定義とSQLの実行」を導入する章と位置付けました。本章では、SQLによって目的を記述し、データベース側がその内容を読み取って処理する入口を作ります。SQLを利用した経験があれば読み進められるように、言語処理の用語は字句解析、構文解析、ASTの順に、実際の変換過程と対応させながら説明します。
...
本章では、これらを字句解析と構文解析を担うParser、ASTを表すStatement、Schema、Catalog、Table、クエリ実行を担うQueryExecutorに分けて実装します。これにより、SQL文字列の解析と、ページファイルに対する読み書きを分離します。
...
この時点で、SQL文字列を扱う責務は字句解析と構文解析を行うParser側に限定されます。クエリ実行以降の処理は、SQLの空白や括弧の位置ではなく、ASTであるStatementが保持しているテーブル名、カラム名、値、条件を参照します。

本章で扱うSQLは、CREATE TABLEとCRUDに必要なINSERT、SELECT、UPDATE、DELETEです。SELECTではFROM、WHERE、一つのJOINを扱います。SQL規格のすべてを実装するのではなく、文字列が実行可能な構造へ変換される流れを確認できる範囲に限定します。
...
入力されたSQL文字列のままでは、実行処理がテーブル名や条件を項目ごとに参照できません。そこで、一般的な言語処理と同じく、@<b>{字句解析}、@<b>{構文解析}、@<b>{ASTの構築}という順序でSQLをプログラムが扱える形へ変換します。

本章の実装では、字句解析をTokenizer、構文解析をSimpleParser、ASTをStatementが担当します。クラス名だけでなく、それぞれが言語処理のどの段階に当たるかを対応させて確認します。
...
クエリ実行は、構文解析が生成したASTを読み取り、実際のデータ操作へ変換する処理です。本章ではQueryExecutorがこの役割を担当します。
...
データの登録、取得、更新、削除というCRUDの意味と、最小構成での処理手順は、第2章「CRUD操作の実装」で説明しています。本章ではCRUD自体を再説明せず、SQLから作られたAST（Statement）、Schema、Catalog、Tableを使って各操作を実行する部分に注目します。
...
REPLは成功時とエラー時のどちらでも、結果に続けて@<code>{(Executed in ... ms)}の形式で実行時間を表示します。計測方法は第4章「処理時間の計測」を参照してください。
...
本章のクエリ実行には、このPlannerがまだありません。第7章では、AST（Statement）から実行計画を作り、条件に応じてインデックス走査と逐次走査を選択する仕組みを扱います。
