...
これらの課題を解決するため、第1章のロードマップで示した通り、本章ではいよいよ「テーブル定義とSQLの実行」を導入します。

一般的なリレーショナルデータベースでは、テーブルごとにカラムの構成を自由に定義できます。たとえば、利用者を保存する@<code>{users}テーブルと、注文を保存する@<code>{orders}テーブルでは、必要なカラムが異なります。
...
ユーザーがこのような標準的なSQLによって「欲しいデータ」を宣言的に記述し、データベース側がその内容を読み取って自動処理する入口を作るため、本章では以下の順序で実装を進めます。

 1. @<b>{リレーショナルモデルとSQL処理の導入}: まずはSQLを受け取ってから実行するまでの、処理全体のパイプラインを整理します。
 2. @<b>{テーブル定義とカタログの設計}: ハードコードされたカラムを廃止し、任意のカラム構成（Schema）を管理できる汎用的なテーブル構造を作ります。
 3. @<b>{字句解析（Tokenizer）の実装}: 入力されたSQL文字列を、プログラムが処理しやすい最小単位（トークン）に分割します。
 4. @<b>{構文解析（Parser）とASTの構築}: トークンの並びからSQLの意味を読み取り、実行可能な「抽象構文木（AST）」へと組み立てます。
 5. @<b>{クエリエグゼキュータによる実行}: ASTの内容に従って、汎用化されたテーブルに対して実際にデータの検索や更新を行います。

これらを実装することで、SQL文字列の解析処理と、ストレージ（ページファイル）に対する読み書き処理が美しく分離され、私たちのプログラムは本格的な「リレーショナルデータベース」へと大きな進化を遂げます。
...
//image[chapter06_image01][][scale=0.3]{
//}
